...
// Sort order starts at 1 (the first teacher) — matches how a viewer counts
// cards top-to-bottom and avoids the "0 looks off" edge case.
...
export default function TeachersPage() {
  ...
  async function load() {
    ...
    try {
      ...
      toast("Could not load the teachers list.", "error");
    } finally {
      setLoading(false);
    }
  }
  ...
  function nextSortOrder(): number {
    const max = teachers.reduce((m, x) => Math.max(m, x.sortOrder ?? 0), 0);
  ...
  function openEditForm(teacher: Teacher) {
    setForm({
      ...
      departmentBn: teacher.departmentBn ?? "",
      imageUrl: teacher.imageUrl ?? "",
      jointDate: teacher.jointDate ?? "",
  ...
  async function handleSubmit() {
    ...
    try {
      const payload = {
        ...
        departmentBn: form.departmentBn.trim() || undefined,
        imageUrl: form.imageUrl || undefined,
        jointDate: form.jointDate || undefined,
        sortOrder: form.sortOrder,
      };
      const res = await fetch(form.id ? `/api/teachers/${form.id}` : "/api/teachers", {
      ...
      toast(form.id ? "Teacher updated." : "Teacher added.");
      closeForm();
      await load();
    } catch (err) {
  ...
  async function handleDelete(teacher: Teacher) {
    const confirmed = await confirmDialog({
      ...
      text: `"${teacher.name}" will be removed from the teachers list.`,
    ...
    try {
      ...
      if (form.id === teacher.id) closeForm();
      await load();
    } catch {
      toast("Could not remove this teacher.", "error");
    }
  }
  ...
  const columns = useMemo<ColumnDef<Teacher, any>[]>(
    () => [
      ...
      { header: "Department", accessorKey: "department" },
      {
        ...
        cell: ({ row }) => {
          const d = row.original.jointDate;
          if (!d) return <span className="text-ink-400">—</span>;
          // jointDate arrives as YYYY-MM-DD; render it in a readable form.
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-500">
            Manage the teachers shown on the public Teachers page.
          </p>
        </div>
        {!formOpen && (
          <button
            ...
            <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="বিভাগ (বাংলা)">
                    <input
                      value={form.departmentBn}
                      onChange={(e) => setForm((prev) => ({ ...prev, departmentBn: e.target.value }))}
                      className={`${INPUT_BASE} border-ink-200`}
                      dir="auto"
                    />
                  </Field>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="Join date" hint="Date the teacher joined the school.">
                    <input
                      type="date"
                      value={form.jointDate}
                      onChange={(e) => setForm((prev) => ({ ...prev, jointDate: e.target.value }))}
                      className={`${INPUT_BASE} border-ink-200`}
                    />
                  </Field>
