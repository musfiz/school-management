...
import type { ExPrincipal } from "@/lib/ex-principals";
...
// Sort order starts at 1 (the first former principal) — this matches how a
// viewer counts cards top-to-bottom and avoids the "0 looks off" edge case.
...
export default function ExPrincipalsPage() {
  const [principals, setPrincipals] = useState<ExPrincipal[]>([]);
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch("/api/ex-principals");
      const data = (await res.json()) as ExPrincipal[];
      setPrincipals(Array.isArray(data) ? data : []);
    } catch {
      toast("Could not load the ex-principals list.", "error");
    } finally {
      setLoading(false);
    }
  }
  ...
  function nextSortOrder(): number {
    ...
    // roster this returns 1 — i.e. the first principal is the first card.
    const max = principals.reduce((m, x) => Math.max(m, x.sortOrder ?? 0), 0);
  ...
  function openEditForm(principal: ExPrincipal) {
    setForm({
      id: principal.id,
      name: principal.name,
      nameBn: principal.nameBn ?? "",
  ...
  async function handleSubmit() {
    ...
    try {
      const payload = {
        ...
        tenure: form.tenure.trim() || undefined,
        imageUrl: form.imageUrl || undefined,
        sortOrder: form.sortOrder,
      };
      const res = await fetch(form.id ? `/api/ex-principals/${form.id}` : "/api/ex-principals", {
      ...
      toast(form.id ? "Principal updated." : "Principal added.");
      closeForm();
      await load();
    } catch (err) {
  ...
  async function handleDelete(principal: ExPrincipal) {
    const confirmed = await confirmDialog({
      title: "Remove this principal?",
      text: `"${principal.name}" will be removed from the ex-principals list.`,
      confirmText: "Yes, remove",
      danger: true,
    });
    if (!confirmed) return;
    try {
      const res = await fetch(`/api/ex-principals/${principal.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
      toast("Principal removed.");
      if (form.id === principal.id) closeForm();
      await load();
    } catch {
      toast("Could not remove this principal.", "error");
    }
  }

  const columns = useMemo<ColumnDef<ExPrincipal, any>[]>(
    () => [
      {
        header: "Photo",
  ...
  return (
    <section className="p-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-ink-900">Ex-Principals</h1>
          <p className="mt-1 text-sm text-ink-500">
            Manage the former principals shown on the public Ex-Principals page.
          </p>
        </div>
        {!formOpen && (
          <button
        ...
        <div className="mb-6">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-display text-lg font-bold text-navy-900">
              {form.id ? "Edit principal" : "Add principal"}
            </h2>
            <button
              type="button"
              onClick={closeForm}
            ...
            <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field
                    label="Tenure"
                    hint="Term of office, e.g. 1990 – 1998."
                  >
                    <input
                      value={form.tenure}
                      onChange={(e) => setForm((prev) => ({ ...prev, tenure: e.target.value }))}
                      className={`${INPUT_BASE} border-ink-200`}
                      placeholder="1990 – 1998"
                    />
                  </Field>
      ...
      {loading ? (
        ...
        <DataTable
          columns={columns}
          data={principals}
          emptyMessage="No former principals yet."
          footer={`Total principals: ${principals.length}`}
        />
      )}
    </section>
  );
