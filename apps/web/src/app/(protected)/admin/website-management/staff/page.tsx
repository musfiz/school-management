...
// Sort order starts at 1 (the first member) — matches how a viewer counts
...
export default function StaffPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(form.id ? `/api/staff/${form.id}` : "/api/staff", {
      ...
      toast(form.id ? "Staff member updated." : "Staff member added.");
      closeForm();
      await load();
    } catch (err) {
  ...
  async function handleDelete(member: StaffMember) {
    const confirmed = await confirmDialog({
      ...
      text: `"${member.name}" will be removed from the staff list.`,
    ...
    try {
      ...
      toast("Could not remove this staff member.", "error");
    }
  }
    ...
    <section className="p-4">
      ...
      {loading ? (
        ...
        <DataTable
          ...
          footer={`Total staff: ${members.length}`}
        />
      )}
    </section>
  );
